...
namespace TMarsupilamiCore.Elements
{
    ...
    public abstract class Element
    {
        ...
        public bool HasUniformSection { get; set; }

        /// <summary>
        /// Returns true if all edge materials are uniform across the element.
        /// </summary>
        /// <remarks>Use this property for fast implementation flavors.</remarks>
        public bool HasUniformMaterial { get; set; }
       ...
       // Mais rappelons nous que courbure + moment => 
        ...
        private Point[] x;          // => pour tous 
        private Vector[] t;         // => par pour le cable, la chaine, la membrane
        private Vector[] d1, d2;    // => par pour le cable, la chaine, la membrane

        // EXTERNAL FORCES & MOMENTS
        private Vector[] _Fext_g;     // (Fx, Fy, Fz) given in the global coordinate system (x,y,z)
        private Vector[] _Mext_m;     // (M1, M2, Q) given in the material coordinate system(d1, d2, t)
        public Vector[] _fext_g;      // linear external moment
        public Vector[] _mext_m;      // linear external moment

        // REACTION MOMENT AND FORCES
        // take care about signs : these are the forces and moments applied
        // by the beam to the supports. 
        public Vector[] _Fr_g;        // (Frx, Fry, Frz) given in the global coordinate system
        public Vector[] _Mr_m;          //   (Mr1, Mr2, Qr) given in the material coordinate system 
        ...
        public abstract Vector[] Get_κb();
        public abstract Vector[] Get_e(Point[] x);
